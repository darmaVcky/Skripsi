...
public class DecisionEngineRouter extends ActiveRouter {
    ...
    public boolean createNewMessage(Message m) {
        if (decider.newMessage(m, interest,getHost())) {
    ...
    public void changedConnection(Connection con) {
        ...
        if (con.isUp()) {
            decider.connectionUp(myHost, otherNode, interest);
            ...
            for (Message m : msgs) {
                if (decider.shouldSendMessageToHost(m, otherNode, interest)) {
                    outgoingMessages.add(new Tuple<Message, Connection>(m, con));
                }
            }
        } else {
            decider.connectionDown(myHost, otherNode, interest);
    ...
    protected void doExchange(Connection con, DTNHost otherHost) {
        conStates.put(con, 1);
        decider.doExchangeForNewConnection(con, otherHost, interest);
    }
    ...
    protected int startTransfer(Message m, Connection con) {
        ...
        if (retVal == RCV_OK) { // started transfer
            ...
            tombstones.add(m.getId());
                ...
                && decider.shouldDeleteOldMessage(m, con.getOtherNode(getHost()), interest)) {
    ...
    public Message messageTransferred(String id, DTNHost from) {
        ...
        boolean isFinalRecipient = decider.isFinalDest(aMessage, getHost(), interest);
        boolean isFirstDelivery = isFinalRecipient
                && !isDeliveredMessage(aMessage);

        if (outgoing != null && decider.shouldSaveReceivedMessage(aMessage, getHost(), interest)) {
    ...
    protected void transferDone(Connection con) {
        ...
        if (decider.shouldDeleteSentMessage(transferred, con.getOtherNode(getHost()), interest)) {
...
//	tryMessagesForConnected(sortByQueueMode(outgoingMessages));
    ...
    protected void findConnectionsForNewMessage(Message m, DTNHost from) {
        ...
        for (Connection c : getConnections()) {
            ...
            if (other != from && decider.shouldSendMessageToHost(m, other, interest)) {
                if (m.getId().equals("M7")) {
